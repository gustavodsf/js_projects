  ...
  componentWillReceiveProps(nextProps) {
    if(typeof nextProps.authenticationError.hasError != 'undefined' &&
       ...
       (typeof this.state.authenticationError.hasError == 'undefined' ||
        ...
        toastr.error(constants.MSG_USER_INAVLID);
    }
  }
  ...
  render() {
    return(
      <div id="container" className="col-sm-9 col-md-6">
        <div id="header-pro">
          <img id="image-logo" src="./assets/image/milk_tool_icon.png" alt="logo"/>
          <span className="title">{constants.PROJECT_NAME}</span>
        </div>
        <form className="form_interface form-horizontal pro-login" onSubmit={this.onLogin.bind(this)}>
          <div className="form-group">
            <label className="control-label col-sm-2">{constants.USER}</label>
          ...
          <div className="form-group">
            <label className="control-label col-sm-2">{constants.PASSWORD}:</label>
            <div className="col-sm-10">
          ...
          <div className="form-group">
            <div className="col-sm-offset-2 col-sm-10">
              <button type="submit" className="btn btn-success">{constants.LOGIN}</button>
            </div>
          </div>
        </form>
